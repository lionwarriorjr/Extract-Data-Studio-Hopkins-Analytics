    ...
    def execute(self, iset, data, params={}, is_grouped=False):
        ...
        if df.shape[0] > 0:
            ...
            for index, row in df.iterrows():
                ...
                #if 'swinging_strike' in row['description']:
                    #ball_type = 'swing_strike'
                #elif 'foul' in row['description']:
                    #ball_type = 'foul'
                #elif row['description'] == 'hit_into_play_no_out':
                    #ball_type = 'hit_into_play_no_out'
                #elif row['description'] == 'hit_into_play':
                    #ball_type = 'hit_into_play_out'
                #elif row['description'] == 'hit_into_play_score':
                    #ball_type = 'hit_into_play_score'
                #elif row['type'] == 'X':
                    #if 'out' in row['event']:
                        #ball_type = 'out'
                    #else:
                ...
                if 'swinging_strike' in row['description']:
                    ...
                    ball_type = 'hit_into_play_out'
                      ...
                      and row['event'] in ['sac_fly','field_out','force_out',
            ...
            for zone in pitches:
                ...
                for pitch_type in pitches[zone]:
                    mapped_df = pitches_mapped_to_zones[zone][pitch_type]
                    swing_strike = pitches[zone][pitch_type]['swing_strike']
                    foul = pitches[zone][pitch_type]['foul']
                    hit_into_play_out = pitches[zone][pitch_type]['hit_into_play_out']
                    hit_into_play_no_out = pitches[zone][pitch_type]['hit_into_play_no_out']
                    #hit_into_play_score = pitches[zone][pitch_type]['hit_into_play_score']
                    hit_into_play = hit_into_play_out + hit_into_play_no_out #+ hit_into_play_score
                    outs = pitches[zone][pitch_type]['out']
                    total = hit_into_play + swing_strike + foul + outs
                    if total > 10:
                        green = (swing_strike + foul)/(swing_strike + foul + hit_into_play)
                        yellow = (swing_strike + foul + hit_into_play_out)/(swing_strike + foul + hit_into_play)
                        red = (hit_into_play_no_out)/(swing_strike + foul + hit_into_play)
                        if red > 0.33:
        ...
        if calc is not None:
            filename = 'pitches_mapped_to_zones_' + batter + '.csv'
